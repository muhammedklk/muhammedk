...
export function ProjectFeature({ project, index = 0 }: { project: Project; index?: number }) { return <article id={`project-${project.slug}`} className={`project-feature${index % 2 ? " is-flipped" : ""}`}><div className="project-head"><span>PROJECT / {project.number}</span><span>{project.category}</span><span>{String(index + 1).padStart(2, "0")} — {String(projects.length).padStart(2, "0")}</span></div><Link to="/work/$slug" params={{ slug: project.slug }} className="project-image" data-cursor="view"><img src={project.image} alt={`${project.title} interface presentation`} loading="lazy" width={project.dimensions[0]} height={project.dimensions[1]} /><span className="project-image-number" aria-hidden="true">{project.number}</span><span className="project-view">VIEW PROJECT <MoveUpRight /></span><span className="project-accent" /></Link><div className="project-info"><span className="project-discipline">{project.category.split("/")[0]}</span><h3>{project.title}</h3><p>{project.description}</p><Link className="case-link" to="/work/$slug" params={{ slug: project.slug }}>EXPLORE CASE STUDY <MoveUpRight /></Link></div></article> }
